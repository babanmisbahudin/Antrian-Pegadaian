...
export default function Penaksir() {
  ...
  const fetchWaiting = useCallback(async () => {
    try {
      const res = await api.get("/queue/waiting/penaksir");
      setWaiting(res.data);
    } catch {
      // silent
    }
  ...
  return (
    <div className="min-h-screen bg-green-100 flex flex-col lg:flex-row gap-4 p-6">
      {/* Panel kiri: kontrol */}
      <div className="bg-white p-6 rounded-2xl shadow-2xl w-full lg:w-1/2 border border-green-300">
        <div className="flex flex-col items-center mb-4">
          ...
          <h1 className="text-xl font-bold text-green-700">Penaksir — Loket {loket}</h1>
        </div>

        <div className="bg-green-200 border border-green-400 rounded-xl py-6 px-4 mb-5 text-center shadow-inner">
        ...
        <div className="flex flex-col gap-3 mb-4">
          <button
            ...
            className="bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl text-lg shadow-md transition"
          >
            {loading
              ? "Memproses..."
              : waiting.length === 0
